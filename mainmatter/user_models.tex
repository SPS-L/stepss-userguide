...
\begin{itemize}
\item the input states are given. They are obtained from the synchronous machine states (see Section~\ref{format-sync}) or the initial power injected into buses (see Section~\ref{sec-refframe})
...
\item the operating-point dependent parameters remain constant, unless they are modified by a user action (see Section~\ref{sec-chgprm}).
\end{itemize}
\end{itemize}
...
{\tt abc\_xyz.f90}. This name has to be used in the data files (see Sections~\ref{format-inj} and~\ref{format-twop}). CODEGEN will correspondingly produce a file named {\tt abc\_xyz.f90} with the FORTRAN code of the model.
