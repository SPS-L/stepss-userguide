...
The nominal system frequency is specified by an FNOM records as follows.
...
\item[$S_{q2}$] = 1 if there is a equivalent winding $q2$, = 0 otherwise
\end{itemize}
...
\begin{figure}[htb]
\centering
\includegraphics[width=69mm]{sync-windings.jpg}~~\includegraphics[width=76mm]{sync-park.jpg}
...
The case of the excitation system is a little more involved and requires specifying an addtional parameter.
...
where:
\begin{itemize}
\item {\tt NAME} is the name of the machine. This is a string of a most 20 characters
...
\item {\tt XPD} is the quadrature-axis transient reactance, in pu
\item {\tt XSD} is the quadrature-axis subtransient reactance, in pu
...
As explained in the preamble of this section, the full model can be simplified by ignoring some of the rotor windings\footnote{let us recall that not all combinations of windings are allowed}. This is specified by replacing the values of some parameters with an asterisk `*' (the quotes are not needed). The parametres of concern are detailed in Table~\ref{tab-sync-simplified}. Let us recall that not all combintations
...
\end{equation}
This is the ratio between the field current in the saturated machine and the corresponding field current when saturation is neglected, for the same operating conditions. Thus, it characterizes the extra excitation current needed in the presence of saturated material. In the above example, saturation was negelected (a zero value was specified for {\tt M}) and, hence, $sat = 1$.

The machine has an excitation controller whose model is named {\tt exc\_GENERIC3}. At the intial operating point the field voltage is $v_f = 2.3680$ pu on the exciter base, which is indirectly defined by the {\tt IBRATIO} parameter of the machine.
